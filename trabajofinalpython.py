...
class Libro:
    def __init__(self, isbn, titulo, autor, año, editorial, categoria, estado):
        self.isbn = isbn
        self.titulo = titulo
        self.autor = autor
        self.año = año
...
class Biblioteca:
    ...
    def cargar(self):
        try:
            ...
            self.libros = []
            for d in datos:
                libro = Libro(d["isbn"], d["titulo"], d["autor"], d["año"], d["editorial"], d["categoria"], d["estado"])
                self.libros.append(libro)
        except Exception as e:
...
class VentanaConsultar:
    ...
    def actualizar(self):
        ...
        for libro in self.biblioteca.listar():
            self.tree.insert("", tk.END, values=(libro.get_isbn(), libro.get_titulo(), libro.get_autor(), libro.get_año(), libro.get_editorial(), libro.get_categoria(), libro.get_estado()))
...
class VentanaModificar:
    def __init__(self, parent, biblioteca):
        ...
        self.entry_año = tk.Entry(self.ventana)
        self.entry_año.pack()
    ...
    def buscar(self):
        try:
            ...
            if libro:
                ...
                self.entry_año.insert(0, str(libro.get_año()))
    ...
    def cargar_archivo(self):
        try:
            archivo = filedialog.askopenfilename(
                title="Seleccionar archivo de biblioteca",
                filetypes=[("Archivos JSON","*.json")]
            )
            if archivo == "":
                return
            self.biblioteca.archivo = archivo
            self.biblioteca.cargar()
            messagebox.showinfo("Cargar", f"Se cargaron {len(self.biblioteca.listar())} libros.")
        except Exception as e:
            messagebox.showerror("Error", str(e))
